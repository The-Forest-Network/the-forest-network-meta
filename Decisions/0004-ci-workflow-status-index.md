...
| `gradle-wrapper-update.yml` | Deleted — same broken fork-skip gate + missing token pattern as `sync-localazy.yml` | [#21](https://github.com/The-Forest-Network/the-forest-network-android/issues/21) → [PR #22](https://github.com/The-Forest-Network/the-forest-network-android/pull/22) |
| `nightlyReports.yml`'s `dependency-analysis` job | Gated to this fork (was running unconditionally); still fails until an NVD API key is added as a secret | [PR #24](https://github.com/The-Forest-Network/the-forest-network-android/pull/24), [android#23](https://github.com/The-Forest-Network/the-forest-network-android/issues/23) |
| Everything else (`build_enterprise.yml`, `build.yml`, `fork-pr-notice.yml`, `generate_github_pages.yml`, `nightly.yml`, `nightlyReports.yml`'s main job, `post-release.yml`, `pull_request.yml`, `quality.yml`, `recordScreenshots.yml`, `release.yml`, `sonar.yml`, `stale-issues.yml`, `triage-incoming.yml`, `triage-labelled.yml`) | **Not reviewed at all** — no equivalent of iOS's #23 audit has been done | — |

## Consequences

- Both repos still have open, known gaps (iOS: 6 recommended-but-not-executed deletions/fixes from ios#23; Android: #19, #20, android#23 (NVD key), and the #18 real fix).
